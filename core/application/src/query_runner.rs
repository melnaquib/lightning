...
impl Drop for QueryRunner {
    ...
    let unix_time = current_time.duration_since(UNIX_EPOCH).expect("Time went backwards!").as_secs();

        error!("We are dropping a query runner at {}. There are {} left", unix_time, count -1);
    }
}
...
impl Clone for QueryRunner{
    fn clone(&self) -> Self {
        ...
        error!("Cloning a query runner. There are now {} query runners", count+1);

        Self { inner: self.inner.clone(), metadata_table: self.metadata_table.clone(), account_table: self.account_table.clone(), client_table: self.client_table.clone(), node_table: self.node_table.clone(), pub_key_to_index: self.pub_key_to_index.clone(), committee_table: self.committee_table.clone(), services_table: self.services_table.clone(), param_table: self.param_table.clone(), current_epoch_served: self.current_epoch_served.clone(), rep_measurements: self.rep_measurements.clone(), latencies: self.latencies.clone(), rep_scores: self.rep_scores.clone(), _last_epoch_served: self._last_epoch_served.clone(), total_served_table: self.total_served_table.clone(), _service_revenue: self._service_revenue.clone(), _commodity_price: self._commodity_price.clone(), executed_digests_table: self.executed_digests_table.clone(), uptime_table: self.uptime_table.clone(), _cid_to_node: self._cid_to_node.clone(), _node_to_cid: self._node_to_cid.clone() }
    }
}
